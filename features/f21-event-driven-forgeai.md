# Event-Driven ForgeAI

> **Feature:** F21  | **Status:** Product specification / Staff Engineer baseline  
...
### Business capability
ForgeAI will react to domain events and trigger AI workflows.
### Business problem
AI should respond to meaningful application events rather than only direct prompts.
### Why this feature exists
AI should respond to meaningful application events rather than only direct prompts. The feature converts this need into a defined product capability with an observable user outcome.
### Capability added
...
### Business outcome
Cause a supported project/task event and verify the correct workflow triggers once with event context. through the normal ForgeAI application path rather than as a standalone Mastra experiment.
...
- Required Mastra capability: **signals, routing, retries, correlation IDs**.
...
**Concepts:** signals, routing, retries, correlation IDs
...
~~~text
┌────────────────────────────────────────────────┐
│ ForgeAI  | Event-Driven ForgeAI             │
├────────────────────────────────────────────────┤
│ Project / context                              │
│ User input / current state                     │
│ ┌────────────────────────────────────────────┐ │
│ │ Cause a supported project/task event and v │ │
│ └────────────────────────────────────────────┘ │
...
|---|---|---|---|
| 1 | Cause a supported project/task event and verify the correct workflow triggers once with event context. | Primary result is produced and shown in UI. | Main business problem is solved end-to-end. |
