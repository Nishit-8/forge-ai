# Workflow Recovery & Debugging

> **Feature:** F09  | **Status:** Product specification / Staff Engineer baseline  
...
### Business capability
ForgeAI will inspect failures and recover workflow runs.

### Business problem
Long-running workflows can fail after partial progress; operators need evidence and controlled recovery.

### Why this feature exists
Long-running workflows can fail after partial progress; operators need evidence and controlled recovery. The feature converts this need into a defined product capability with an observable user outcome.
...
### Business outcome
Open a failed/suspended run, inspect errors/context, then retry, resume or replay an appropriate point. through the normal ForgeAI application path rather than as a standalone Mastra experiment.
...
- Required Mastra capability: **snapshots, suspended runs, resume, time travel, error handling**.
...
**Concepts:** snapshots, suspended runs, resume, time travel, error handling
...
~~~text
┌────────────────────────────────────────────────┐
│ ForgeAI  | Workflow Recovery & Debugging    │
├────────────────────────────────────────────────┤
│ Project / context                              │
│                                                │
│ User input / current state                     │
│ ┌────────────────────────────────────────────┐ │
│ │ Open a failed/suspended run, inspect error │ │
│ └────────────────────────────────────────────┘ │
...
|---|---|---|---|
| 1 | Open a failed/suspended run, inspect errors/context, then retry, resume or replay an appropriate point. | The primary result is produced and shown in the UI. | The main business problem is solved end-to-end. |
...
**Mastra dependency:** Phase 7 — Workflow Reliability and Debugging.  
