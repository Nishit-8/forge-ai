...
> **Feature:** F08  | **Status:** Product specification / Staff Engineer baseline  
...
### Business capability
ForgeAI will view and understand workflow executions.

### Business problem
Durable workflows require user-facing visibility into what ran, its state, and its result.

### Why this feature exists
Durable workflows require user-facing visibility into what ran, its state, and its result. The feature converts this need into a defined product capability with an observable user outcome.
...
### Business outcome
Open the run center, select a run, and inspect status, stages, timestamps and results. through the normal ForgeAI application path rather than as a standalone Mastra experiment.
...
- Required Mastra capability: **run status, streaming, snapshots, result inspection**.
...
**Concepts:** run status, streaming, snapshots, result inspection
...
~~~text
┌────────────────────────────────────────────────┐
│ ForgeAI  | Workflow Run Center              │
├────────────────────────────────────────────────┤
│ Project / context                              │
│                                                │
│ User input / current state                     │
│ ┌────────────────────────────────────────────┐ │
│ │ Open the run center, select a run, and ins │ │
│ └────────────────────────────────────────────┘ │
...
|---|---|---|---|
| 1 | Open the run center, select a run, and inspect status, stages, timestamps and results. | The primary result is produced and shown in the UI. | The main business problem is solved end-to-end. |
...
**Mastra dependency:** Phase 4/7 — Workflow Results and Observability.  
