...
> **Feature:** F22  | **Status:** Product specification / Staff Engineer baseline  
...
### Business capability
ForgeAI will run approved AI workflows on a schedule.
### Business problem
Recurring operational work should not depend on users remembering to start it manually.
### Why this feature exists
Recurring operational work should not depend on users remembering to start it manually. The feature converts this need into a defined product capability with an observable user outcome.
### Capability added
**Observable scheduled automation.**
...
### Business outcome
Create a schedule, verify timezone/next run, execute, pause/resume/delete it and inspect trigger history. through the normal ForgeAI application path rather than as a standalone Mastra experiment.
...
- Required Mastra capability: **schedule API, cron, timezone, persistence, adapters**.
...
**Concepts:** schedule API, cron, timezone, persistence, adapters
...
~~~text
┌────────────────────────────────────────────────┐
│ ForgeAI  | Scheduled AI Workflows           │
├────────────────────────────────────────────────┤
│ Project / context                              │
│ User input / current state                     │
│ ┌────────────────────────────────────────────┐ │
│ │ Create a schedule, verify timezone/next ru │ │
│ └────────────────────────────────────────────┘ │
...
|---|---|---|---|
| 1 | Create a schedule, verify timezone/next run, execute, pause/resume/delete it and inspect trigger history. | Primary result is produced and shown in UI. | Main business problem is solved end-to-end. |
