# AI Tool-Enabled Assistant

> **Feature:** F03  | **Status:** Product specification / Staff Engineer baseline  
...
### Business capability
ForgeAI will let the assistant safely perform project operations.

### Business problem
A conversational assistant becomes useful when it can invoke controlled business capabilities instead of only generating text.

### Why this feature exists
A conversational assistant becomes useful when it can invoke controlled business capabilities instead of only generating text. The feature converts this need into a defined product capability with an observable user outcome.

### Capability added
**Governed AI action layer.**
...
### Business outcome
Ask the assistant to inspect or change an allowed project resource and inspect the tool result. through the normal ForgeAI application path rather than as a standalone Mastra experiment.
...
- Required Mastra capability: **Tools, schemas, multiple tools, hooks, result transformation**.
...
**Concepts:** Tools, schemas, multiple tools, hooks, result transformation
...
~~~text
┌────────────────────────────────────────────────┐
│ ForgeAI  | AI Tool-Enabled Assistant        │
├────────────────────────────────────────────────┤
│ Project / context                              │
│                                                │
│ User input / current state                     │
│ ┌────────────────────────────────────────────┐ │
│ │ Ask the assistant to inspect or change an  │ │
│ └────────────────────────────────────────────┘ │
...
|---|---|---|---|
| 1 | Ask the assistant to inspect or change an allowed project resource and inspect the tool result. | The primary result is produced and shown in the UI. | The main business problem is solved end-to-end. |
...
**Mastra dependency:** Phase 2 — Tool Orchestration.  
