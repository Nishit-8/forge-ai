...
> **Feature:** F16  | **Status:** Product specification / Staff Engineer baseline  
...
### Business capability
ForgeAI will run long AI work without holding an interactive request.
### Business problem
Long tasks should continue after a user navigates away and remain inspectable.
### Why this feature exists
Long tasks should continue after a user navigates away and remain inspectable. The feature converts this need into a defined product capability with an observable user outcome.
### Capability added
**Durable asynchronous AI execution.**
...
### Business outcome
Start a background task, leave, return later, inspect progress/result/failure and retry if appropriate. through the normal ForgeAI application path rather than as a standalone Mastra experiment.
...
- Required Mastra capability: **background tasks, lifecycle, durable execution**.
...
**Concepts:** background tasks, lifecycle, durable execution
...
~~~text
┌────────────────────────────────────────────────┐
│ ForgeAI  | Background AI Tasks              │
├────────────────────────────────────────────────┤
│ Project / context                              │
│ User input / current state                     │
│ ┌────────────────────────────────────────────┐ │
│ │ Start a background task, leave, return lat │ │
│ └────────────────────────────────────────────┘ │
...
|---|---|---|---|
| 1 | Start a background task, leave, return later, inspect progress/result/failure and retry if appropriate. | Primary result is produced and shown in UI. | Main business problem is solved end-to-end. |
...
**Mastra:** Phase 12 — Background Tasks and Long-running Execution.  
